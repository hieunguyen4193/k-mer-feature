...
main_outputdir="/Volumes/HNSD01/outdir/highdepth_WGS_TSS_customGenes/fastq";
...
for enrichment in all_genes up_genes down_genes;do \
    ...
    for inputbam in ${all_bams}; do \
        ...
        samtools fastq -@ 8 ${inputbam} \
            -1 ${outputdir}/${sampleid}_R1.fastq.gz \
            -2 ${outputdir}/${sampleid}_R2.fastq.gz;
        done;done