...
for inputbam in ${all_input_bams};do \
    ...
    samtools view -b -h ${inputbam} -L ${inputbed} > ${outputdir}/${bedname}/${filename%.bam*}.filtered.bam;
    ...
    for i in $(seq 1 $nregions); do \
        ...
        samtools view -b -h ${inputbam} -L $region  > ${outputdir}/${bedname}/${regionname}/${filename%.bam*}.filtered.bam;
